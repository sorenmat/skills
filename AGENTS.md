...
See the `git` skill (auto-loads when relevant). Core rules: never push to `master`/`main`, never auto-merge PRs, branch-based development, plan before non-trivial action. Also, never include LLM attribution (e.g., `Co-authored-by:` lines naming an AI assistant or model, `[codex]` tags, or `generated by AI` notes) in commit messages, PR titles/descriptions, or comments.
